...
def tunnel_ready_after_restart(config: dict, reason: str, node: str | None) -> int:
    """Report whether Cloudflared actually registered with the Cloudflare edge.

    A service in state `Running` only means the process started. If it never
    registers, the public hostname serves 1033 while the log still shows a
    clean restart. This only looks and tells the truth: choosing nodes stays
    with the operator, so nothing here tries to repair a bad choice.
    """
    ready, endpoint = wait_for_ready(config, float(config.get("tunnel_ready_timeout_seconds", 45)))
    log_event(
        "tunnel_ready_check",
    ...
    while True:
        try:
            ...
            if reason:
                ...
                ready = tunnel_ready_after_restart(config, reason, current_node)
